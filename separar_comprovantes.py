    ...
    for pagina in paginas.pages:
        ...
        if contador_nome_pessoa >= 2:
            ...
            if not linha_do_nome.empty:
                ...
                liquido = "{:,.2f}".format(liquido).replace(",", "X").replace(".", ",").replace("X", ".") 
                e_sobrenome = ""
                ...
                for palavra in texto_pagina.split():
                    ...
                    if len(palavra) > 3 and palavra in nome.split():
                        ...
                        if len(palavras_encontradas) >= 2 and e_sobrenome in partes_nome[-1] and str(liquido) in texto_pagina:
...
diretorio = r'C:\Users\mateus.santos.AD\Documents\SEPARAR_NOME_E_LIQUIDO'
